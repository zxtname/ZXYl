...
namespace ZXYL.DAL
{
    public partial class BaseDataDapperContrib<T> where T : class ,new()
    {
        ...
        /// <returns></returns>
        public List<T> GetList(string where,object parms=null, string sort = null, int limits = -1, string fileds = " * ", string orderby = "")
        {
            var tableName = typeof(T).Name;
            ...
            using (SqlConnection cn = new SqlConnection(DapperHelper.ConnStr))
            {
                return cn.Query<T>(sql.ToString(), param:parms).ToList();
            }
