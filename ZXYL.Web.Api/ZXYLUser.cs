...
{
    public static class ZXYLFactory
    {
        public static ZXYLUser GetZXYLUser(this IPrincipal user)
        {
            if (!user.Identity.IsAuthenticated)
            {
                return null;
            }
            return new ZXYLUser(user);
        }
    }

    public class ZXYLUser
    {
        public ZXYLUser(IPrincipal user)
        {
            this.User = user;
